...
<html xmlns="https://www.w3.org/1999/xhtml">
	...
	<body style="margin:0px;">
		...
		<script type="text/javascript">
			{literal}		
				function valid(inValue,tipo){
					var salida=inValue;
					if(inValue.includes(' ')){
						salida=inValue.split(' ');
						salida=tipo===1?salida[0]:salida[1];
					} 
					if(inValue.includes(':')){
						salida=inValue.split(':');
						salida=tipo===1?salida[0]:salida[1];
				...
				function GV_Map() {
					...
					{/literal}				
						{foreach item=Row from=$Rows name=RowsGrid}
							{literal}
								GV_Draw_Marker({
									lat:{/literal}valid('{$Row.Latitude}',1),
									lon:valid('{$Row.Longitude}',2),
									name:'{$Row.IdLum}',
									desc:'FRFTIPO DE LUMINARIA:{$Row.TipoDeLuminaria}--TIPO DE CALLE:{$Row.TipoDeCalle}--TIPO DE VIA:{$Row.Via}--FECHA CAPTURA:{$Row.FechaCaptura}--MUNICIPIO:{$Row.Municipio}--COLONIA:{$Row.Colonia}--CALLE:{$Row.Calle}--CARGA ACEPTADA:{$Row.CargaAceptada}--LIGA A FOTOGRAFIA:    https://emihermx.com/toluca2021/{$Row.Fotografia}',
									label:'{$Row.TipoLuminaria}',
