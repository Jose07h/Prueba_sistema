...
{foreach item=Row from=$Rows name=RowsGrid}
	{literal}GV_Draw_Marker({lat:{/literal}{$Row.Latitude},lon:{$Row.Longitude},name:'{$Row.IdLum}',desc:'TIPO DE LUMINARIA:{$Row.TipoDeLuminaria}--TIPO DE CALLE:{$Row.TipoDeCalle}--TIPO DE VIA:{$Row.Via}--FECHA CAPTURA:{$Row.FechaCaptura}--MUNICIPIO:{$Row.Municipio}--COLONIA:{$Row.Colonia}--CALLE:{$Row.Calle}--CARGA ACEPTADA:{$Row.CargaAceptada}    LIGA A FOTOGRAFIA:    https://emihermx.com/toluca2021/{$Row.Fotografia}',label:'{$Row.TipoLuminaria}',color:'',icon:''{literal}});{/literal}
